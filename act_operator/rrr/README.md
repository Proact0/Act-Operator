...
```bash
$ git clone https://github.com/Proact0/rrr.git
$ cd rrr
```
...
>
> ```bash
> $ uv sync --package rrrr
> ```

- langgraph.json에 노드 수정 (예: rrrr만 설치할 경우)
...
```json
{
  ...
  "graphs": {
    ...
    "rrrr": "./casts/rrrr/workflow.py:rrrr_workflow"
  },
  "env": ".env"
}
```
...
- `GraphState`는 `casts/rrrr/modules/state.py:rrrr_state`에서 개별 관리됩니다.

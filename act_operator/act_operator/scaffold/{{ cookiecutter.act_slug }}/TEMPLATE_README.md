...
|-------|---------|-------------|
| `@act-architect` | Design architecture | Planning new cast, unclear about structure, need CLAUDE.md |
| `@cast-developer` | Implement code | Building nodes/agents/tools, need LangGraph patterns |
| `@act-engineer` | Project setup | Creating cast package, adding dependencies, syncing env |
| `@cast-tester` | Write tests | Creating pytest tests, mocking strategies, fixtures |
...
1. **Manual invocation**: Type `@skill-name` in your prompt to explicitly load context.
   ```
   @act-architect Help me design a RAG pipeline
   ```
...
- `act-architect`: Interactive Q&A → generates `CLAUDE.md`
- `cast-developer`: Reads `CLAUDE.md` (Optional) → implements code
- `act-engineer`: Manages packages and dependencies
- `cast-tester`: Creates pytest test files

### Recommended Development Flow

```
1. @act-architect  →  Design & create CLAUDE.md
        ↓
2. @act-engineer   →  Create cast, add dependencies
        ↓
3. @cast-developer   →  Implement nodes, agents, graphs
        ↓
4. @cast-tester      →  Write and run tests
```
...
> - For **agent-to-agent communication**, use the Mermaid charts in `CLAUDE.md` generated by the `@act-architect` skill.
...
| 스킬 | 목적 | 사용 시점 |
|------|------|----------|
| `@act-architect` | 아키텍처 설계 | 새 cast 계획, 구조 불명확, CLAUDE.md 필요 시 |
| `@act-engineer` | 프로젝트 설정 | cast 패키지 생성, 의존성 추가, 환경 동기화 |
| `@cast-developer` | 코드 구현 | 노드/에이전트/툴 구현, LangGraph 패턴 필요 시 |
| `@cast-tester` | 테스트 작성 | pytest 테스트 생성, 모킹 전략, 픽스처 |
...
1. **수동 호출**: 프롬프트에 `@skill-name`을 입력하여 명시적으로 컨텍스트를 로드합니다.
   ```
   @act-architect RAG 파이프라인 설계를 도와줘
   ```
...
- `act-architect`: 대화형 Q&A → `CLAUDE.md` 생성
- `cast-developer`: `CLAUDE.md` 읽기(선택) → 코드 구현
- `act-engineer`: 패키지 및 의존성 관리
- `cast-tester`: pytest 테스트 파일 생성

### 권장 개발 흐름

```
1. @act-architect  →  설계 & CLAUDE.md 생성
        ↓
2. @act-engineer   →  (필요 시) cast 생성, 의존성 추가
        ↓
3. @cast-developer   →  노드, 에이전트, 그래프, 기타 모듈 구현
        ↓
4. @cast-tester      →  테스트 작성 및 실행
```
...
> - **에이전트 간의 소통**: `act-architect` 스킬로 생성된 `CLAUDE.md`의 Mermaid 차트를 사용하세요.
